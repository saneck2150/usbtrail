...
function(usbtrail_add_bpf)
    ...
    if(NOT EXISTS "/sys/kernel/btf/vmlinux")
        message(FATAL_ERROR
            "USBTRAIL_BUILD_BPF=ON requires /sys/kernel/btf/vmlinux. "
    ...
    if(CMAKE_LIBRARY_ARCHITECTURE)
        list(APPEND BPF_INCLUDE_ARGS "-I/usr/include/${CMAKE_LIBRARY_ARCHITECTURE}")
    endif()

    add_custom_command(
        OUTPUT "${BPF_OBJ}"
        COMMAND "${USBTRAIL_CLANG_EXECUTABLE}"
                -g -O2 -target bpf
    ...
    add_custom_command(
        OUTPUT "${BPF_SKEL}"
        COMMAND "${PROJECT_SOURCE_DIR}/scripts/generate-skeleton.sh"
                "${USBTRAIL_BPFTOOL_EXECUTABLE}" "${BPF_OBJ}" "${BPF_SKEL}"
        DEPENDS "${BPF_OBJ}" "${PROJECT_SOURCE_DIR}/scripts/generate-skeleton.sh"
        COMMENT "Generating ${BPF_TARGET}.skel.h"
        VERBATIM
    )

    add_custom_target(${BPF_TARGET} ALL DEPENDS "${BPF_OBJ}" "${BPF_SKEL}")
endfunction()
